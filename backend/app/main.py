...
def parse_playlist_item(item: Dict[str, Any]) -> Optional[schemas.PlaylistBase]:
    ...
    if not spotify_url: return None
...
def save_playlist_for_user(playlist: schemas.PlaylistCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def get_suggestions(request: schemas.SuggestionRequest):
    ...
    try:
        ...
        playlists = [parsed for item in spotify_playlists if (parsed := parse_playlist_item(item)) is not None]
